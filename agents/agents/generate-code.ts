...
function buildPrompt(skillName: string, language: string, count: number): { role: string; content: string }[] {
  return [
    { role: 'system', content: `你是编程教练，擅长通过渐进式练习让学生真正掌握代码。为「${skillName}」设计 ${count} 个渐进式代码练习。
...
{"examples":[{"title":"","description":"","difficulty":"basic","language":"","setup":"","task":"","hint":"","solution":"","solutionExplanation":[],"expectedOutput":"","commonMistakes":[],"keyPoints":[],"relatedConcepts":[]}],"bestPractices":[],"commonMistakes":[]}` },
    { role: 'user', content: `为「${skillName}」生成 ${count} 个 ${language} 代码练习` },
  ];
}

function parseResponse(raw: string, skillName: string, language: string, count: number): CodeData {
  try {
    ...
    const examples: CodeExample[] = Array.isArray(data.examples)
      ? data.examples.slice(0, count).map((item: any) => ({
          ...
          solutionExplanation: Array.isArray(item.solutionExplanation) ? item.solutionExplanation.slice(0, 5).map((e: any) => String(e)) : [],
          expectedOutput: String(item.expectedOutput || ''),
          commonMistakes: Array.isArray(item.commonMistakes) ? item.commonMistakes.slice(0, 3).map((m: any) => String(m)) : [],
          keyPoints: Array.isArray(item.keyPoints) ? item.keyPoints.slice(0, 3).map((p: any) => String(p)) : [],
          relatedConcepts: Array.isArray(item.relatedConcepts) ? item.relatedConcepts.slice(0, 3).map((c: any) => String(c)) : [],
        }))
      : [];

    return {
      skill: skillName, language, totalExamples: examples.length, examples,
      bestPractices: Array.isArray(data.bestPractices) ? data.bestPractices.slice(0, 5).map((p: any) => String(p)) : [],
      commonMistakes: Array.isArray(data.commonMistakes) ? data.commonMistakes.slice(0, 3).map((m: any) => String(m)) : [],
    };
  } catch {
    return { skill: skillName, language, totalExamples: 0, examples: [], bestPractices: [], commonMistakes: ['生成失败'] };
  }
}

export async function generateCode(
  skillName: string, language: string = 'JavaScript', count: number = 3, llmConfig?: LLMConfig
  ...
  const messages = buildPrompt(skillName.trim(), language, count);
  const raw = await callLLM(llmConfig!, messages);
  const result = parseResponse(raw, skillName.trim(), language, count);
