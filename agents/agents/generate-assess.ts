...
function buildPrompt(
  ...
  return [
    { role: 'system', content: `你是学习效果评估专家，负责分析学生学习数据并给出评估。
...
function parseResponse(raw: string, goal: string): AssessData {
  try {
    ...
    return {
      overallScore: 0, level: '评估失败', dimensions: [], weakPoints: [], improvements: [],
      planAdjustment: '', encouragement: '', summary: '评估失败，请重试',
    };
  }
}
...
export async function assessLearning(
  learningData: string,
  goal: string = '掌握技术栈',
  ...
  if (!learningData?.trim()) {
    return { type: 'error', data: { message: '请提供学习数据（如：已完成课程、做题情况、学习时长等）' } };
  }

  const messages = buildPrompt(learningData.trim(), goal, currentProgress);
  const raw = await callLLM(llmConfig!, messages);
  const result = parseResponse(raw, goal);

  return { type: 'assessment', data: result };
}
