...
export class LectureAgentService {
  ...
  /**
   * 生成讲义
   * @param skillName 技能名称（如 "React Hooks"）
  ...
  async generate(
    ...
    const params = { skillName: skillName.trim(), level, extra };

    // 尝试从缓存获取
    const cacheKey = this.cacheService.generateKey('LectureAgent', 'generate', params);
    const cached = await this.cacheService.get<LectureData>(cacheKey);
    if (cached) {
      console.log(`[LectureAgent] 缓存命中: ${skillName}`);
    ...
    const lectureData = this.parseResponse(result.content, skillName.trim(), level);
    ...
    // 写入缓存（1小时）
    await this.cacheService.set(cacheKey, lectureData, 3600);
...
- 练习题 5-7 道（比以前更多更丰富），包含至少 1 道概念反转题和 1 道迁移题`;
  ...
  private parseResponse(raw: string, skillName: string, level: string): LectureData {
    ...
    if (exerciseMatch) {
      ...
      exercises.push(...this.parseExercises(exerciseMatch[1]));
    }
    ...
    return {
      ...
      content,
      exercises,
      keyPoints: this.extractSection(content, '知识点总结', 5),
      prerequisites: this.extractSection(content, '前置知识', 5),
  ...
  private parseExercises(text: string): Exercise[] {
    ...
    for (let i = 0; i < blocks.length; i++) {
      ...
      const question = block.substring(
        typeMatch ? block.indexOf(']') + 1 : 0,
        answerIdx > 0 ? answerIdx : block.length,
      ).trim();
  ...
  private extractSection(content: string, sectionName: string, maxItems: number): string[] {
