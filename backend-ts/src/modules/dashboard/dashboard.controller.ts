...
export class DashboardController {
  ...
  async getDashboard(@CurrentUser() user: any, @Query('user_id') userIdStr?: string) {
    ...
    const result = await this.dashboardService.getDashboard(userId, Number(user.tenantId) || 1);
  ...
  async getTodayActions(@CurrentUser() user: any) {
    const result = await this.dashboardService.getTodayActions(user.sub, Number(user.tenantId) || 1);
    return success(result);
  }
  ...
  async getGrowthReport(@CurrentUser() user: any, @Query('days') days?: string) {
    const result = await this.dashboardService.getGrowthReport(user.sub, Number(days || 30), Number(user.tenantId) || 1);
    return success(result);
  }
}
