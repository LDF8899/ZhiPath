...
async function main() {
  ...
  const sql = fs.readFileSync(path.join(rootDir, 'deploy', '20260718-evaluation-spine.sql'), 'utf8');
  const config = {
  ...
  try {
    ...
    console.log(`evaluation spine migration complete on ${config.host}:${config.port}/${config.database}`);
  } finally {
    await conn.end();
  }
}
...
main().catch((err) => {
  console.error(`evaluation spine migration failed: ${err.message}`);
  process.exit(1);
});
