...
import { getAdminResumes, reviewResume } from '../../api/admin';
...
export default function AdminResumes() {
  ...
  const fetchData = useCallback(async (p = page, st = filter) => {
    setLoading(true);
    try { const res = await getAdminResumes({ page: p, pageSize: 20, status: st }); setData(res.data || []); setTotal(res.total || 0); }
  ...
  const handleReview = async (id: number, status: number) => {
    try {
      await reviewResume({ id, status, review_comment: status === 1 ? '审核通过' : '审核拒绝' });
  ...
  const columns: AdminColumn[] = [
    ...
    { key: 'title', title: '简历标题', render: (v) => v || '-' },
    { key: 'status', title: '状态', render: (v) => { const s = statusLabel[v] || statusLabel[0]; return <span className={`hd-badge ${s.cls}`}>{s.text}</span>; } },
    { key: 'reviewComment', title: '审核意见', render: (v) => v || '-' },
    { key: 'createTime', title: '提交时间', sortable: true, render: (v) => v ? new Date(v).toLocaleDateString('zh-CN') : '-' },
  ...
  return (
    <div className="hd-canvas">
      <AdminPageHeader title="简历管理" subtitle={`共 ${total} 份简历`} actions={
