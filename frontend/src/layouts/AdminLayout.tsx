...
export default function AdminLayout() {
      ...
      {navItems.map((item) => {
        ...
        return (
          <li key={item.key}>
            ...
            >
              <span className="hd-sidebar-icon"><Icon size={18} /></span>
              {item.label}
            </button>
          </li>
        );
  ...
  const footer = (
    <div className="hd-sidebar-footer">
      <div className="hd-flex" style={{ marginBottom: 12, gap: 10 }}>
        <div className="hd-avatar small">{user?.realName?.[0] || 'A'}</div>
        <div style={{ minWidth: 0 }}>
          <div style={{ font: '14px/1.2 var(--hand-bold)', color: 'var(--ink)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
            {user?.realName || '管理员'}
          </div>
          <div style={{ font: '11px/1 var(--mono)', color: 'var(--pencil)', marginTop: 2 }}>admin</div>
        </div>
      </div>
      <button onClick={handleLogout} className="hd-sidebar-item" style={{ color: 'var(--accent)' }}>
        <span className="hd-sidebar-icon"><IconLogout size={18} /></span>
        退出登录
      </button>
    </div>
  );
      ...
      <div className="hd-layout">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', minWidth: 0 }}>
          <header className="hd-mobile-header">
            ...
            <span style={{ font: '700 16px/1 var(--hand-bold)', color: 'var(--ink)' }}>
              智途管理后台
            </span>
            <div className="hd-avatar small" style={{ background: 'var(--accent)', color: 'var(--paper)' }}>
              {user?.realName?.[0] || 'A'}
            </div>
          </header>
